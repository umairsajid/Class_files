...
template<typename Item_Type>
  ...
  */
  ...
  // Insert rotate_left here
    /*<exercise chapter="11" section="1" type="programming" number="1">*/
    /** rotateLeft
	pre:  local_root is the root of a binary search tree
	post: local_root->right is the root of a binary search tree
              local_root->right->right is raised one level
              local_root->right->left does not change levels
              local_root->left is lowered one level
              local_root is set to the new root
       @param local_root The root of the binary tree to be rotated
    */
